...
export const DashboardOverview = ({ setActiveTab }) => {
  ...
  return (
    <div className="animate-fade-in" style={{ padding: '28px', maxWidth: '1440px', margin: '0 auto' }}>
      ...
      {/* Role-Based Access Control (RBAC) User Permission Showcase Card */}
      <div className="glass-panel" style={{
        padding: '22px 28px',
        marginBottom: '28px',
        background: '#ffffff',
        border: `2px solid ${deptInfo?.border || '#bae6fd'}`,
        borderRadius: '16px',
        boxShadow: '0 4px 18px rgba(2, 132, 199, 0.07)'
      }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '14px', marginBottom: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            <span style={{ fontSize: '2.6rem' }}>{user?.avatar || '👨‍💼'}</span>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                ...
                <span style={{
                  ...
                  {deptInfo?.badge || user?.roleLabel || user?.role}
                </span>
                <span style={{ fontSize: '0.85rem', color: '#64748b', fontWeight: 700 }}>
                  ({user?.email})
                </span>
              </div>
              <p style={{ color: '#475569', fontSize: '0.92rem', marginTop: '3px', fontWeight: 600 }}>
                {deptInfo?.description || 'ระบบคลังสินค้าอัตโนมัติ 3D AS/RS'}
              </p>
            </div>
          </div>
          ...
          >
            ...
            <span>สลับบทบาทเพื่อทดสอบสิทธิ์</span>
          </button>
        </div>

        {/* 5 Permission Chips Matrix */}
        <div style={{
          display: 'grid',
          ...
          {deptInfo?.capabilities?.map((cap) => (
            <div
              ...
              style={{
                ...
                padding: '10px 14px',
                borderRadius: '12px',
                background: cap.allowed ? '#f0fdf4' : '#fff1f2',
                border: `1.5px solid ${cap.allowed ? '#86efac' : '#fecdd3'}`,
                color: cap.allowed ? '#15803d' : '#be123c'
              }}
            >
              {cap.allowed ? (
                <CheckCircle2 size={20} color="#16a34a" style={{ flexShrink: 0 }} />
              ) : (
                <Lock size={18} color="#e11d48" style={{ flexShrink: 0 }} />
              )}
              <div style={{ minWidth: 0 }}>
                <div style={{ fontWeight: 800, fontSize: '0.85rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {cap.name}
                </div>
                <div style={{ fontSize: '0.74rem', fontWeight: 700, opacity: 0.9 }}>
                  {cap.note}
                </div>
              </div>
            </div>
